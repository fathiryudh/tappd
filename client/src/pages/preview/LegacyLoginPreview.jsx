...
export default function LegacyLoginPreview() {
  const { login } = useAuth()
      ...
      <div className="flex-1 flex flex-col justify-center px-8 md:px-16 lg:px-20 bg-zinc-950">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            >
              {loading ? 'Signing in...' : <>Sign in <ArrowRight size={15} weight="bold" /></>}
            </motion.button>
          </form>

          <p className="mt-6 text-sm text-zinc-600">
            No account?{' '}
            <Link to="/preview/legacy/register" className="text-zinc-300 hover:text-zinc-100 font-medium transition-colors duration-150">
              Register
            </Link>
          </p>
        </MotionDiv>
